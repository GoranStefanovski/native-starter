<script lang="ts" setup>
  ...
  const getPostUri = `guest/common/${id}/getBlogPost`
  ...
  const postUri = computed(() => edit ? `common/blog-post/${id}/edit` : '/common/save-blog-post');
  ...
  const postImg = (() => {
    ...
    if (media != undefined) {
      const postImage = media.find(o => o.collection_name === 'blog_post_image');
      if (!!postImage) {
  ...
  const beforeSubmit = (hasToRedirect = true) => {
    onSubmit(postUri.value, 'posts', hasToRedirect, form.value);
  }
